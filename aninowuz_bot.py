...
async def get_telegram_image(file_id):
    try:
        # 1. httpx bilan asinxron mijoz ochish
        async with httpx.AsyncClient() as client:
            ...
            if not file_info.get('ok'):
                return "Fayl topilmadi yoki file_id noto'g'ri", 404
                
            file_path = file_info['result']['file_path']
            
            # 2. Haqiqiy rasm faylini asinxron yuklab olish
        ...
        logger.error(f"Tasvir yuklashda xatolik: {e}")
...
def get_admin_kb(is_main=False):
    ...
    if is_main:
        buttons.append([InlineKeyboardButton("👮 Adminlarni boshqarish", callback_data="manage_admins")],
                       [InlineKeyboardButton("🧹 Admin logni tozalash", callback_data="clear_admin_logs"), InlineKeyboardButton("Admin logl", callback_data="adm_logs")],
                       [InlineKeyboardButton("🔁 Bazani qayta o'rnatish", callback_data="reset_database")]
...
async def admin_channels_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    # Callback query javobini berish (soat belgisi ketishi uchun)
    await query.answer()
    
    try:
        # Eng yangi ro'yxatni bazadan olish
        # dictionary=True natijani {'username': '...', 'subscribers_added': 0} ko'rinishida beradi
        async with db_pool.acquire() as conn:
            async with conn.cursor(dictionary=True) as cur:
                await cur.execute("SELECT username, subscribers_added FROM channels")
                channels = await cur.fetchall()
        
        text = "📢 <b>Majburiy obuna kanallari boshqaruvi</b>\n\n"
        
        if not channels:
            text += "⚠️ <i>Hozircha majburiy obuna uchun kanallar qo'shilmagan.</i>"
        else:
            text += "<b>Hozirgi kanallar ro'yxati:</b>\n"
            for i, ch in enumerate(channels, 1):
                # Har bir kanal va undan kelgan obunachilar soni
                text += f"{i}. {ch['username']} — 👤 {ch['subscribers_added']} ta\n"
                
        text += "\n<i>Kerakli amalni tanlang:</i>"
                
        keyboard = [
            [
                InlineKeyboardButton("➕ Qo'shish", callback_data="add_ch_start"),
                InlineKeyboardButton("❌ O'chirish", callback_data="rem_ch_start")
            ],
            [
                InlineKeyboardButton("📊 Umumiy Statistika", callback_data="channel_stats"),
                InlineKeyboardButton("📃 Barcha kanallar", callback_data="view_all_channels")
            ],
            [
        ...
        logger.error(f"Admin kanal menyusida xato: {e}")
        await query.message.reply_text("❌ Ma'lumotlarni yuklashda xatolik yuz berdi.")
        return A_MAIN
# =============================================================================================
...
async def exec_add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("❌ Bekor qilish", callback_data="manage_admins"), InlineKeyboardButton("🔙 Orqaga", callback_data="manage_admins")]
    ]
...
# ===================================================================================
    ...
    # --- REMOVE ANIME BO'LIMI ---
        ...
        kb = [
            ...
            [ InlineKeyboardButton("🔙 ORQAGA", callback_data="rem_ani_menu"),InlineKeyboardButton("❌ BEKOR QILISH", callback_data="rem_ani_menu")]
        ]
        await query.edit_message_text(
...
# ===================================================================================
   ...
   # 1. VIP o'chirish ro'yxatini chiqarish
    ...
    # 2. KANALLAR BOSHQARUVI ASOSIY MENYUSI
    elif data == "adm_ch":
        keyboard = [
            [
                InlineKeyboardButton("➕ Qo'shish", callback_data="add_ch_start"),
            ...
            [
                InlineKeyboardButton("📊 Kanallar Statistika", callback_data="channel_stats"),
                InlineKeyboardButton("📃 Barcha kanallar", callback_data="view_all_channels")
            ],
            [
                InlineKeyboardButton("🔙 Orqaga", callback_data="admin_main")
    ...
    elif data == "add_ch_start": # Nom to'g'rilandi
    ...
    # 4. Faqat o'chirish uchun ro'yxat (Statistikasiz)
    elif data == "rem_ch_start": # Nom to'g'rilandi
        channels = await get_all_channels() 
        if not channels:
            await query.answer("❌ Kanallar mavjud emas", show_alert=True)
            return None
        ...
        for ch in channels:
            ...
            # Sizning xohishingizga ko'ra (+raqam) olib tashlandi
        ...
        await query.edit_message_text("🗑 <b>O'chirmoqchi bo'lgan kanalni tanlang:</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

    # 5. Barcha kanallarni inline tugma qilib ko'rsatish
    elif data == "view_all_channels":
        channels = await get_all_channels()
        if not channels:
            await query.answer("📭 Kanallar ro'yxati bo'sh", show_alert=True)
            return None
        
        keyboard = []
        for ch in channels:
            ch_name = ch['username'] if isinstance(ch, dict) else ch[1]
            # Tugma bosilganda kanalga o'tish imkoniyati
            url = f"https://t.me/{ch_name.replace('@', '')}"
            keyboard.append([InlineKeyboardButton(f"🔗 {ch_name}", url=url)])
        
        keyboard.append([InlineKeyboardButton("⬅️ Orqaga", callback_data="adm_ch")])
        await query.edit_message_text("📃 <b>Barcha majburiy kanallar ro'yxati:</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")

    # 6. Kanallar statistikasi
    elif data == "channel_stats":
        channels = await get_all_channels()
        if not channels:
            await query.answer("📊 Statistika mavjud emas", show_alert=True)
            return None
        
        stat_text = "📊 <b>Kanallar bo'yicha obuna statistikasi:</b>\n\n"
        for ch in channels:
            ch_name = ch['username'] if isinstance(ch, dict) else ch[1]
            sub_count = ch.get('subscribers_added', 0) if isinstance(ch, dict) else 0
            stat_text += f"🔹 <b>{ch_name}</b>: 👤 {sub_count} ta obunachi\n"
        
        stat_text += "\n<i>Ushbu raqamlar bot orqali kanalga qo'shilgan foydalanuvchilar sonini ko'rsatadi.</i>"
        
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Orqaga", callback_data="adm_ch")]])
        await query.edit_message_text(stat_text, reply_markup=kb, parse_mode="HTML")

    # 7. Kanalni o'chirish ijrosi
        ...
        # Ro'yxatni yangilash uchun "rem_ch_start" callback'ini qayta chaqiramiz
        query.data = "rem_ch_start"
...
async def search_anime_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        # Qidiruv rejimlarini sozlash (Xabarni o'zgartirish orqali)
        mode_map = {
            "search_type_name": ("🔍 Anime <b>nomini</b> yozing:", "name"),
            "search_type_id": ("🆔 Anime <b>ID raqamini</b> yozing:", "id"),
            "search_type_character": ("👤 <b>Personaj</b> yoki tavsif yozing:", "character")
        }

        if data in mode_map:
            text, mode = mode_map[data]
            context.user_data["search_mode"] = mode
            # Eski xabarni tahrirlaymiz
            await query.edit_message_text(
                text=text, 
                parse_mode="HTML", 
                reply_markup=get_cancel_kb() # "⬅️ Orqaga" tugmasi bo'lishi shart
            )
            return A_SEARCH_BY_NAME
            ...
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    res = await cur.fetchone()
                    if res:
                        # Random tanlanganda ham xabarni tahrirlab ko'rsatamiz
                        return await show_selected_anime(update, context, res['anime_id'])
            return A_SEARCH_BY_NAME

        elif data == "cancel_search":
            # Qidiruvdan butunlay chiqish
            await query.message.delete() # Qidiruv oynasini o'chiramiz
            await query.message.reply_text("🏠 Asosiy menyu", reply_markup=get_main_kb(user_id))
            return ConversationHandler.END
    ...
    # "Orqaga" yoki "Bekor qilish" kelsa - qidiruv menyusiga qaytish (END emas!)
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                if text.isdigit() or search_type == "id":
                    ...
                    sql = "SELECT * FROM anime_list WHERE name LIKE %s OR original_name LIKE %s LIMIT 20"
                    params = (f"%{text}%", f"%{text}%")
       ...
       # NATIJA TOPILMAGANDA
        ...
        # NATIJA KO'P BO'LSA (Tugmalar ro'yxati)
        keyboard = []
        for anime in results:
            r_sum, r_count = anime.get('rating_sum') or 0, anime.get('rating_count') or 0
            rating = round(r_sum / r_count, 1) if r_count > 0 else "N/A"
            keyboard.append([InlineKeyboardButton(f"🎬 {anime['name']} ⭐ {rating}", callback_data=f"show_anime_{anime['anime_id']}")])
        
        # Ro'yxat ostiga "Orqaga" tugmasini qo'shish chatni toza saqlashga yordam beradi
        keyboard.append([InlineKeyboardButton("⬅️ Qidiruvga qaytish", callback_data="search_type_name")])
...
async def search_anime_by_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.get('result'):
            ...
            async with db_pool.acquire() as conn:
                async with conn.cursor(dictionary=True) as cur:
                    # AI qaytargan nomni qidiruvga tayyorlaymiz
                    # LIKE o'rniga MATCH() AGAINST() ishlatamiz
                    ...
                    await cur.execute(sql_query, (clean_name,))
