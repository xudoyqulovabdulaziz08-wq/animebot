...
async def create_fan_profile(uid: int, bio: str, fav_genre: str):
    """
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    ...
                    (uid, f"Yangi kanal qo'shdi: {username}")
                )
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, f"Kanalni o'chirdi: {username}")
                )
    ...
    if data == "admin_main":
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Admin panel bosh menyusiga qaytdi")
                )
            ...
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if res:
                        ...
                        await cur.execute(
                            "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                            (uid, f"Animega qism qo'shishni boshladi: {res['name']} (ID: {ani_id})")
                        )
                    else:
...
# ===================================================================================
   ...
   # 1. VIP o'chirish ro'yxatini chiqarish
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                        (uid, f"Foydalanuvchidan VIP maqomini oldi: {target_id}")
                    )
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                        (uid, f"Foydalanuvchiga VIP maqomi berdi: {target_id}")
                    )
    ...
    # 5. Kanalni o'chirish ijrosi
        ...
        await delete_channel_by_id(ch_id, admin_id=uid) 
...
# ===================================================================================
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Yangi anime qo'shish jarayonini boshladi")
                )
    ...
    # 3. ADMIN PANELGA QAYTISH
        ...
        # Status yuqorida await get_user_status(uid) orqali olingan
        is_main = (status == "main_admin")
    ...
    # 4. REKLAMA GURUHI TANLANGANDA
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, f"Reklama yuborishni boshladi (Guruh: {target_group})")
                )
    ...
    # 1. BAZANI EKSPORT QILISH (21-band: Audit log bilan)
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Bazani JSON formatda eksport qildi (Backup)")
                )
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                        (uid, f"Adminlikdan olib tashladi: {adm_id}")
                    )
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                        (uid, f"Yangi admin tayinladi: {new_id}")
                    )
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                        (uid, f"Foydalanuvchini VIP ro'yxatidan o'chirdi: {target_id}")
                    )
...
async def show_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT bonus, status FROM users WHERE user_id=%s", (uid,))
                res = await cur.fetchone()
    ...
    try:
        ...
        if not results:
            ...
            await update.message.reply_text(
                ...
                f"<i>💡 Maslahat: Anime nomini o'zbek yoki ingliz tilida yozib ko'ring.</i>",
        ...
        note = ""
        if len(results) > 20:
            note = f"\n\n⚠️ <i>Natijalar juda ko'p. Aniqroq nom yozishingizni so'raymiz.</i>"

        await update.message.reply_text(
            f"🔍 <b>'{text}' bo'yicha topilgan animelar:</b>{note}",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
        )
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Anime boshqaruv paneliga kirdi")
                )
    except Exception as e:
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Kontent qo'shish paneliga kirdi")
                )
    except Exception as e:
...
async def select_ani_for_ep(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                (uid, "Qism qo'shish uchun anime tanlash bo'limiga kirdi")
            )
...
async def list_animes_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                (uid, f"Anime ro'yxatini ko'zdan kechirdi (Sahifa: {page + 1})")
            )
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, f"Qism qo'shish uchun ro'yxatni ko'rdi (Sahifa: {page+1})")
                )
    except Exception as e:
    ...
    if user_text == ADVERTISING_PASSWORD:
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, "Reklama paneliga parol orqali kirdi")
                )
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, f"Reklama parolini noto'g'ri kiritdi: {user_text[:10]}...")
                )
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO admin_logs (admin_id, action) VALUES (%s, %s)",
                    (uid, f"Baza eksport qilindi ({len(animes)} ta anime)")
                )
        ...
        await msg.reply_document(
            ...
            caption=(
                ...
                f"👤 <b>Eksport qildi:</b> Admin (ID: {uid})"
            ),
            parse_mode="HTML"
        )
...
async def start_profile_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_subscribed = await check_user_subscription(uid, context.bot)
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor(dictionary=True) as cur:
                ...
                await cur.execute("SELECT name FROM users WHERE user_id = %s", (uid,))
                user = await cur.fetchone()
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor(dictionary=True) as cur:
                ...
                await cur.execute("SELECT bonus, name FROM users WHERE user_id = %s", (uid,))
                user = await cur.fetchone()
                ...
                # 2. Ballarni ayirish
                await cur.execute("UPDATE users SET bonus = bonus - %s WHERE user_id = %s", (cost, uid))
                
                if item_type == 'vip':
                    # VIP muddatini hisoblash va yangilash
                    # Avvalgi darslarda yozgan add_vip_logic funksiyamizni chaqiramiz
                    new_expire = await add_vip_logic(uid, value) 
                    ...
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"👤 <b>Foydalanuvchi:</b> {user['name']} (ID: {uid})\n"
...
async def send_donation_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_invoice(
            ...
            payload=f"donate_{amount_uzs}_{uid}", # To'lovni tekshirish uchun ma'lumot
