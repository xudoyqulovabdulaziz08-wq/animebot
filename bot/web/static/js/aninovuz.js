...
const translations = {
  uz: {
    ...
    historytext: "Bu yerda siz oxirgi marta ko'rgan animelaringiz saqlanadi.",
    nohistory: "Siz hali birorta ham anime ko'rmagansiz.",
    abouttitle: "Aninovuz Loyihasi Haqida",
    abouttext:
      "O'zbekistondagi eng innovatsion anime platformasiga xush kelibsiz. Bizning maqsadimiz — sifatli kontentni har bir muxlisga yetkazishdir.",
    yoursection: "Siz uchun",
    missiontitle: "Bizning Missiyamiz",
    missiontext:
      "Aninovuz loyihasi 2025-yilda anime ixlosmandlari uchun qulay muhit yaratish maqsadida tashkil etilgan. Biz nafaqat anime ko'rish, balki o'zbek tili rivojiga hissa qo'shadigan sifatli dublyaj va subtitrlarni taqdim etishni o'z oldimizga maqsad qilganmiz. Bizning platformamiz Telegram WebApp texnologiyasi asosida qurilgan bo'lib, bu sizga ilova o'rnatmasdan turib, brauzer tezligida anime ko'rish imkonini beradi.",
    feat1title: "Tezkorlik",
    feat1desc:
      "Bizning serverlarimiz MySQL bazasi bilan optimallashtirilgan bo'lib, ma'lumotlar soniyalar ichida yuklanadi.",
    feat2title: "Xavfsizlik",
    feat2desc:
      "Foydalanuvchi ma'lumotlari Telegram xavfsizlik protokollari orqali himoyalangan.",
    feat3title: "Sifat",
    feat3desc:
      "Eng so'nggi animelar FHD formatda va o'zbekcha tarjimada taqdim etiladi.",
    guidetitle: "Botdan qanday foydalaniladi?",
    step1:
      "Telegramdagi <strong>@aninovuz_bot</strong> manziliga o'ting va /start buyrug'ini bosing.",
    step2: "Bot menyusidan 'Anime katalogi' bo'limiga o'ting.",
    step3:
      "Qiziqqan anime nomini qidiruv orqali toping yoki janr bo'yicha ko'rib chiqing.",
    statanimes: "Animelar",
    statusers: "Foydalanuvchilar",
    statsupport: "Texnik yordam",
    contactus: "Savollaringiz bormi?",
    contacttext:
      "Agar sizda hamkorlik yoki loyiha bo'yicha takliflar bo'lsa, biz bilan bog'laning.",
  ...
  ru: {
    ...
    historytext: "Здесь сохраняется история ваших последних просмотренных аниме.",
    nohistory: "Вы еще не смотрели ни одного аниме.",
    abouttitle: "О проекте Aninovuz",
    abouttext:
      "Добро пожаловать на самую инновационную аниме-платформу в Узбекистане. Наша цель — предоставлять качественный контент каждому поклоннику.",
    missiontitle: "Наша миссия",
    missiontext:
      "Проект Aninovuz был создан в 2025 году с целью создания удобной среды для любителей аниме. Мы стремимся не только предоставить возможность смотреть аниме, но и внести вклад в развитие узбекского языка, предлагая качественный дубляж и субтитры. Наша платформа построена на технологии Telegram WebApp, что позволяет вам смотреть аниме с скоростью браузера без необходимости установки приложений.",
    feat1title: "Скорость",
    feat1desc:
      "Наши серверы оптимизированы с помощью MySQL базы данных, обеспечивая загрузку данных за считанные секунды.",
    feat2title: "Безопасность",
    feat2desc:
      "Данные пользователей защищены с помощью протоколов безопасности Telegram.",
    feat3title: "Качество",
    feat3desc:
      "Последние аниме представлены в формате FHD с узбекским дубляжом.",
    guidetitle: "Как использовать бот?",
    step1:
      "Перейдите по адресу <strong>@aninovuz_bot</strong> в Telegram и нажмите команду /start.",
    step2: "Перейдите в раздел 'Каталог аниме' в меню бота.",
    step3:
      "Найдите интересующее вас аниме с помощью поиска или просмотрите по жанрам.",
    statusers: "Пользователи",
    statanimes: "Аниме",
    statsupport: "Техническая поддержка",
    contactus: "Есть вопросы?",
    contacttext:
      "Если у вас есть предложения по сотрудничеству или проекту, свяжитесь с нами.",
  ...
  en: {
    ...
    historytext: "This is where your recently watched anime will be saved.",
    nohistory: "You haven't watched any anime yet.",
    abouttitle: "About Aninovuz Project",
    abouttext:
      "Welcome to the most innovative anime platform in Uzbekistan. Our goal is to deliver quality content to every fan.  ",
    ...
    feat1title: "Speed",
    feat1desc:
      "Our servers are optimized with a MySQL database, ensuring data loads within seconds.",
    feat2title: "Security",
    feat2desc: "User data is protected through Telegram's security protocols.",
    feat3title: "Quality",
    feat3desc:
      "The latest anime are presented in FHD format with Uzbek dubbing.",
    guidetitle: "How to use the bot?",
    step1:
      "Go to <strong>@aninovuz_bot</strong> on Telegram and press the /start command.",
    step2: "Go to the 'Anime Catalog' section in the bot menu.",
    step3:
      "Find the anime you are interested in through search or browse by genre.",
    statusers: "Users",
    statanimes: "Animes",
    statsupport: "Technical Support",
    contactus: "Have questions?",
    contacttext:
      "If you have proposals for cooperation or projects, contact us.",
...
function renderAnimes() {
  ...
  animeData.forEach((anime) => {
    ...
    if (anime.category === "popular" && popularGrid)
      popularGrid.innerHTML += cardHTML;
    if (anime.category === "foryou" && forYouGrid)
      forYouGrid.innerHTML += cardHTML;
  });
}
...
searchInput.addEventListener("input", async (e) => {
  ...
  try {
    ...
    if (data.length === 0) {
      searchResultsGrid.innerHTML =
        '<p style="padding: 20px;">Hech narsa topilmadi...</p>';
      return;
    }
...
function openAdmin(event) {
  event.preventDefault(); // Brauzer odatiy ochishini to'xtatamiz

  const adminUrl = "https://t.me/Khudoyqulov_pg";

  // Telegram WebApp maxsus funksiyasi orqali ochish
  if (window.Telegram && window.Telegram.WebApp) {
    window.Telegram.WebApp.openTelegramLink(adminUrl);
  } else {
    // Agar WebApp tashqarisida bo'lsa (oddiy brauzer)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  navLinks.forEach((link) => {
    // Linkning href qiymatini olamiz
    const linkPath = link.getAttribute("href");

    if (
      currentPath === linkPath ||
      (currentPath === "/" && linkPath.includes("aninovuz.html"))
    ) {
...
// Tarixni yuklash uchun kichik mantiq
        document.addEventListener('DOMContentLoaded', () => {
                ...
                history.forEach(anime => {
                    grid.innerHTML += `
                        <div class="anime-card">
                            <img src="${anime.image}" alt="${anime.title}">
                            <h3>${anime.title}</h3>
                            <p style="font-size: 0.7rem; color: var(--text-muted);">Ko'rilgan: ${anime.date}</p>
                            <a href="https://t.me/aninovuz_bot?start=ani_${anime.id}" class="watch-btn">
                                <i class="fa-brands fa-telegram"></i>
                                <span data-lang="watch_bot">Botda ko'rish</span>
                            </a>
                        </div>
                    `;
                });
...
function addToHistory(id, title, image) {
  let history = JSON.parse(localStorage.getItem("animeHistory") || "[]");
  const newEntry = { id, title, image, date: new Date().toLocaleDateString() };

  // Dublikatlarni tekshirish
  history = history.filter((item) => item.id !== id);
  history.unshift(newEntry); // Eng birinchi qatorga qo'shish

  localStorage.setItem("animeHistory", JSON.stringify(history.slice(0, 20))); // Faqat oxirgi 20tasini saqlash
}
