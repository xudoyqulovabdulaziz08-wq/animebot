...
@router.callback_query(F.data.startswith("list_anime_contine_page:"))
async def process__contine_anime_list_page(callback: CallbackQuery, session: Any):
    page = int(callback.data.split(":")[1])
    
    # Ma'lumotlarni bazadan/keshdan yuklaymiz
    markup, total_count = await get_anime_contine_list_markup(session, page=page)
    
    text = (
        f"📋 {html.bold('Bazadagi davom etayotgan animelar ')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📊 Davom etayotgan anime soni: {html.code(total_count)} ta\n"
...
async def get_anime_contine_list_markup(session: Any, page: int = 1, per_page: int = 10) -> tuple[InlineKeyboardMarkup, int]:
    from services.anime_service import AnimeService
    ...
    try:
        data = await service.get_ongoing_animes(offset=offset, limit=per_page)
        total_anime = data.get("total_count", 0)
        animes = data.get("animes", [])
    except Exception as e:
        logger.error(f"❌ Ogoin animelar ro'yxatini olishda xatolik: {e}")
        total_anime = 0
        animes = []
