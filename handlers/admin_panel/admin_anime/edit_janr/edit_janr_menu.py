...
@router.callback_query(lambda c: c.data == "janr_menu")
async def genre_menu_handler(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    ...
    await safe_answer(callback)  # CallbackQuery to'g'ri uzatildi
    await _safe_update_message(message=callback.message, caption=text, reply_markup=kb)