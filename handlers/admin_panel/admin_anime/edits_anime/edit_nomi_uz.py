...
class EditAnimeStates(StatesGroup):
    waiting_for_new_name_uz = State()
    waiting_for_confirmation_uz = State()    # Yangi nomni kiritish holati uzbekcha
...
# =====================================================================
# 📑 1-QADAM: "🇺🇿 Uzbekcha" tugmasi bosilganda
# =====================================================================
@router.callback_query(F.data.startswith("edit_field:title_uz:"))
async def edit_anime_title_uz_start(callback: CallbackQuery, state: FSMContext):
    try:
    ...
    await state.set_state(EditAnimeStates.waiting_for_new_name_uz)
    ...
    text = (
        "✍️ <b>Yangi O'zbekcha nom kiritish:</b>\n\n"
        "Iltimos, animening yangi nomini yozib, xabar shaklida yuboring."
    )
    
    await callback.answer("O'zbekcha nomni tahrirlash...")
...
# =====================================================================
@router.message(EditAnimeStates.waiting_for_new_name_uz, F.text)
async def process_new_anime_name_uz(message: Message, state: FSMContext):
    ...
    await state.update_data(new_anime_title_uz=new_name)
    await state.set_state(EditAnimeStates.waiting_for_confirmation_uz)
    
    # ❌ style="success" kabi parametrlar Aiogram v3 da InlineKeyboardButton uchun ISHLAMAYDI. Olib tashlandi.
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Ha", callback_data="confirm_edit_uz:yes", style="success"),
            InlineKeyboardButton(text="❌ Yo'q", callback_data="confirm_edit_uz:no", style="danger")
        ]
    ])
    
    confirm_text = (
        f"❓ <b>Haqiqatdan ham o'zbekcha nom quyidagiga o'zgartirilsinmi?</b>\n\n"
...
# =====================================================================
@router.callback_query(EditAnimeStates.waiting_for_confirmation_uz, F.data.startswith("confirm_edit_uz:"))
async def save_or_cancel_anime_title_uz(callback: CallbackQuery, state: FSMContext, session: Any):
    action = callback.data.split(":")[1]
    state_data = await state.get_data()
    
    anime_id = state_data.get("edit_anime_id")
    new_name = state_data.get("new_anime_title_uz")
    ...
    try:
        # DIQQAT: Biz bazaga "title_uz" ni uzatayapmiz. Service buni to'g'ri qabul qilishi kerak!
        success = await service.update_anime(anime_id=anime_id, update_data={"title_uz": new_name})
    except Exception as e:
        logger.error(f"DB Update error title_uz: {e}", exc_info=True)
        success = False
    ...
    await callback.message.edit_caption(
        caption=f"✅ <b>Anime o'zbekcha nomi muvaffaqiyatli yangilandi!</b>\n\n✨ Yangi nom: <code>{new_name}</code>",
...
async def force_refresh_edit_nom_menu(callback: CallbackQuery, state: FSMContext, session: Any):
    ...
    cloned_callback = callback.model_copy(update={"data": f"edit_fields:{anime_id}"})
    from handlers.admin_panel.admin_anime.edits_anime.edit_anime_nomi import edit_fields_callback
    await edit_fields_callback(cloned_callback, session)
