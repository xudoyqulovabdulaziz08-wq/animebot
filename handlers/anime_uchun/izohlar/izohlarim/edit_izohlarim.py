...
async def safe_call(coro, *, context: str = "") -> None:
    """Boshqa handlerni (masalan anime_comment_handler / handle_my_comments) xavfsiz chaqiradi."""
    try:
        await coro
    except TelegramForbiddenError:
        pass
...
async def handle_my_comments(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
            ...
            if "message is not modified" in err_msg:
                ...
                try:
                    ...
                    if "message is not modified" not in str(e2).lower():
                        logger.error(f"❌ handle_my_comments fallback edit xatosi: {e2}")
                ...
                try:
                    ...
                    logger.warning(f"handle_my_comments: yangi xabar yuborilmadi: {e2}")
            else:
        ...
        logger.error(f"❌ handle_my_comments kutilmagan xatolik: {e}", exc_info=True)
...
async def handle_comment_replies(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        # 2. Bazadan ketma-ket ma'lumotlarni olish
        ...
        try:
            ...
            if "message is not modified" in error_msg:
                ...
                try:
                    ...
                    logger.warning(f"handle_comment_replies: yangi xabar yuborilmadi: {e2}")
            else:
...
async def edit_comment_start_handler(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        logger.error(f"❌ Bazadan izohni olishda xatolik: {err}", exc_info=True)
        await safe_answer(callback, "❌ Tizimda xatolik yuz berdi.", show_alert=True)
        return

    if not comment or comment.get("user_id") != callback.from_user.id:
        await safe_answer(callback, "❌ Izoh topilmadi yoki bu sizning izohingiz emas!", show_alert=True)
        return
    ...
    try:
        ...
        if "message is not modified" in error_msg:
            ...
            try:
                ...
                logger.warning(f"edit_comment_start_handler: prompt yuborilmadi: {e2}")
                await state.clear()
        else:
            logger.error(f"❌ TelegramBadRequest yuz berdi: {e}")
    except TelegramForbiddenError:
        ...
        logger.error(f"❌ Kutilmagan xatolik: {err}", exc_info=True)
...
async def process_new_comment_text(message: types.Message, state: FSMContext, bot):
    ...
    # 🟢 TUZATILDI: state ma'lumotlari yo'qolgan bo'lsa (masalan bot qayta ishga tushgan),
    # message_id=None bilan edit_message_* chaqirish TelegramBadRequest beradi — oldindan tekshiramiz.
    if not prompt_message_id or not anime_id or not comment_id:
        await state.clear()
        return
    ...
    if not user_text or len(user_text) < 2:
        # Matn bo'lmagan (rasm, ovoz va h.k.) yoki juda qisqa xabar — foydalanuvchiga bildiramiz
        try:
            await message.answer("✍️ Iltimos, kamida 2 ta belgidan iborat matn yuboring.")
        except (TelegramBadRequest, TelegramForbiddenError):
            pass
        return
    ...
    try:
        ...
        if "message is not modified" in error_msg:
            return
        if "there is no caption in the message to edit" not in error_msg and "message has no caption" not in error_msg:
            logger.warning(f"❌ Caption edit qilishda xato: {e}")
    except TelegramForbiddenError:
        await state.clear()
        return
    except Exception as err:
    ...
    # Fallback: caption emas, matnli xabar bo'lishi mumkin
    try:
        ...
        if "message is not modified" not in str(e).lower():
            logger.warning(f"❌ Text edit qilishda xato: {e}")
    except TelegramForbiddenError:
        await state.clear()
        return
    except Exception as err:
        logger.error(f"❌ Text edit kutilmagan xatolik: {err}", exc_info=True)

    # Ikkalasi ham ishlamasa — foydalanuvchi osilib qolmasligi uchun yangi xabar yuboramiz
    try:
        new_msg = await message.answer(text=preview_text, reply_markup=keyboard, parse_mode="HTML")
...
async def confirm_edit_comment_handler(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not new_text or not anime_id:
        await safe_answer(callback, "⚠️ Izoh matni topilmadi. Qaytadan urinib ko'ring.", show_alert=True)
        await state.clear()
        return

    # 🔒 Qayta bosish (double-click) natijasida update_comment ikki marta yuborilmasligi uchun
    #    state'ni DB yozuvidan OLDIN tozalaymiz.
    await state.clear()
    ...
    try:
        ...
        if success:
            await safe_answer(callback, "✅ Izohingiz muvaffaqiyatli yangilandi!", show_alert=True)
            ...
            await safe_call(
                ...
                context="confirm_edit_comm -> handle_my_comments"
            )
        else:
            await safe_answer(callback, "❌ Izohni yangilashda xatolik yuz berdi.", show_alert=True)
